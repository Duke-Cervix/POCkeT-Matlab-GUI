function varargout = Spanish_Cervix_Clinic_Liga(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Spanish_Cervix_Clinic_Liga_OpeningFcn, ...
                   'gui_OutputFcn',  @Spanish_Cervix_Clinic_Liga_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function varargout = Spanish_Cervix_Clinic_Liga_OutputFcn(hObject, eventdata, handles) 
...
if (TF == 1) 
     warningMessage = sprintf('Error: por favor escribe identificación del paciente');
         uiwait(warndlg(warningMessage));
         return; % Get out and don't process any further.
elseif (TFSTR == 1)
    warningMessage = sprintf('Error: La identificación del paciente solamente debe consistir en números.');
         uiwait(warndlg(warningMessage));
         return; % Get out and don't process any further.
else
...
switch val
    ...
    case 1
         warningMessage = sprintf('Error: Por favor, seleccione procedimiento');
         uiwait(warndlg(warningMessage));
         return; % Get out and don't process any further.
    case 2
...
if((exist(fullpathname, 'file') == 7) && (numTimesPressed == 1))
     warningMessage = sprintf('Warning: Usted ya ha escrito en esta identificación del paciente. Recuperar la imagen si esta es la correcta identificación del paciente.');
         uiwait(warndlg(warningMessage));
         return; % Get out and don't process any further.
...
for i = 1:images_per_click
    ...
    axes(handles.image)
    %imshow(im); 
    imwrite(im,[num2str(counter),'Blanca_PC_Gen4_AA_WD_35mm_',study_site,'-',patientType,'0',num2str(patientID),'-',num2str(i),'noComp300dpi','.tif'],'Compression','none','Resolution',300);
end
...
cd(key_directories{1});
h=msgbox('Completado luz blanca captura de imágenes');
...
for i = 1:images_per_click
    ...
    axes(handles.image)
    %imshow(im); 
    imwrite(im,[num2str(counter),'Verde_PC_Gen4_AA_WD_35mm_',study_site,'-',patientType,'0',num2str(patientID),'-',num2str(i),'noComp300dpi','.tif'],'Compression','none','Resolution',300);
end
...
% Change this line of code if you want to change folder the matlab files are saved in
 %%cd('C:\Users\Cervix 9\Desktop\Test\')
 cd(key_directories{1});
 h=msgbox('Completado luz verde captura de imágenes');
guidata(hObject,handles)
...
% Change this line of code if you want to change folder the matlab files are saved in
 ...
 h=msgbox('Completado Lugol captura de imágenes');
guidata(hObject,handles)
